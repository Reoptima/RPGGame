...
void registration() {
    ...
    while (set_grade) {
        clear();
        switch (choice("Выбирите класс", list<string>{"Воин", "Лучник", "Маг", "Наёмник"})) {
            case 1:
                Hero = WarriorClass(name);
                set_grade = false;
                break;
            case 2:
                Hero = ArcherClass(name);
                set_grade = false;
                break;
            case 3:
                Hero = MagClass(name);
    ...
    while (set_skill) {
        ...
        switch (choice("Выбирите один скилл", skills)) {
            case 1:
                Hero.setSkill(Hero.getAllSkillsGrade()[0]);
                set_skill = false;
                break;
            case 2:
                Hero.setSkill(Hero.getAllSkillsGrade()[1]);
                set_skill = false;
                break;
            case 3:
                Hero.setSkill(Hero.getAllSkillsGrade()[2]);
                set_skill = false;
                break;
        }
    }
