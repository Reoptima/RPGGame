...
void fight(HeroClass &hero, vector<EnemyClass> enemies) {
    ...
    while (battle) {
        ...
        info = "Ход " + to_string(enemyAttack + 1) + "\nПротивники:\n";
        ...
        while (menuChoice) {
            switch (choiceWhile(info, list<string>{"Наёмники", "Скилы", "Быстрая атака"})) {
                case 1:
                    if (hero.getArmies().empty()) {
                        clear();
                        std::cout << "Нет наёмников" << std::endl;
                        Sleep(1000);
                        break;
                    }
                    army = listArmies();
        ...
        if (IsGoal) {
            ...
            for (auto enemy : enemies) {
                enemiesInfo.push_back(
                        enemy.getName() + " HP " + to_string(int(100.0 / enemy.getMaxHP() * enemy.getHP())) + "%");
            }
            enemies[choiceWhile("Выберите противника ", enemiesInfo) - 1].dealt_damage(damage, element);
        } else {
            for (auto &enemy : enemies) {
...
int fight_Boss(HeroClass &hero, HeroClass boss) {
    ...
    while (battle) {
        ...
        info += "\nВы:\nHP: " + to_string(hero.getHP()) + "/" + to_string(hero.getMaxHPAll()) +
                "; Мана: " + to_string(hero.getMana()) + "/" + to_string(hero.getMaxManaAll());
        while (menuChoice) {
            switch (choiceWhile(info, list<string>{"Наёмники", "Скилы", "Быстрый удар"})) {
                case 1:
                    if (hero.getArmies().empty()) {
                        clear();
                        std::cout << "Наёмники отсутсвуют" << std::endl;
                        Sleep(1000);
                        break;
                    }
                    army = listArmies();
...
ArmyClass listArmies() {
    ...
    return Hero.getArmies()[choiceWhile("Наёмники", armies) - 1];
}
