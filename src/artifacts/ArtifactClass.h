...
// ArtifactClass(Название, Тип, HP, Мана, ФизУрон, МагУрон, Защита, Интеллект, Цена);
...
public:
    ...
    std::string getInfo() const {
        ...
        text+= this->optionally_magic_power!=0?"Маг.урон:"+std::to_string(this->optionally_magic_power)+"\n":"";
        text+= this->optionally_defense!=0?"Защита:"+std::to_string(this->optionally_defense)+"\n":"";
        text+= this->optionally_knowledge!=0?"Интеллект:"+std::to_string(this->optionally_knowledge)+"\n":"";
        text+= "За "+std::to_string(this->gold)+" золота";
        return text;
    }
